...
public static class Threshold
{
    ...
    /// <returns></returns>
    public static int GetMinimumThreshold(int[] HistGram, int[] HistGramS, ref int n_mutlpyY, int[] mutlpyY)
    {
        int Y, Iter = 0;
        ...
        for (Y = 26; Y < 230; Y++)
        {
            //Console.WriteLine("HistGram[Y]" + Y + " " + HistGram[Y]); 
            HistGram[Y] = HistGram[Y] - 100;
            if (HistGram[Y] > 0)
            {
        ...
        // 通过三点求均值来平滑直方图
        while (IsDimodal(HistGramCC, peaks) == false)                                        // 判断是否已经是双峰的图像了      
        {
            //Console.WriteLine("IsDimodal(HistGramCC);" + IsDimodal(HistGramCC, peaks)); 
            ...
            Iter++;
            if (Iter >= 120) break;                                                   // 直方图无法平滑为双峰的，返回错误代码
        }
        Console.WriteLine("Iter" + Iter + "peaks" + peaks[0] + " " + peaks[1]);
        for (Y = 26; Y < 230; Y++) HistGramS[Y] = (int)HistGramCC[Y];
        // 阈值极为两峰之间的最小值 
        for (Y = 1; Y < 255; Y++)
        {
            //Iter < 150 还是使用双点谷底法
           ...
           {
             */
               //Iter >= 150 说明无法平滑到双点，使用多点谷底法
               if (HistGramCC[Y - 1] <= HistGramCC[Y] && HistGramCC[Y + 1] <= HistGramCC[Y])
               {
                   if (HistGramCC[Y] > 10)
                   {
                       Console.WriteLine("peakCount=" + peakCount + " Y= " + Y);
                       peaks[peakCount] = Y;
                       peakCount++;//比真实数量大1
                   }
               }
               //findPeaks(HistGramCC, ref peakCount, peaks);
                /*
               double left = HistGramCC[Y - 1]-HistGramCC[Y];
               double right = HistGramCC[Y + 1]-HistGramCC[Y];
               Console.WriteLine(left + " "+right);
               if ( left>0 && right>0 )
               {
                   Console.WriteLine("n_mutlpyY" + n_mutlpyY + "Y" + Y);
                   Console.WriteLine("HistGramCC[Y - 1]=" + HistGramCC[Y - 1] + " HistGramCC[Y]=" + HistGramCC[Y] + " HistGramCC[Y + 1]=" + HistGramCC[Y + 1]);
                   mutlpyY[n_mutlpyY] = Y;
                   n_mutlpyY++;
               }
                 */
        ...
        for (int p = 1; p < peakCount; p++)
        {
            int left = peaks[p - 1];
            int right = peaks[p];
            double minHistGram = 10000;
            int minHistPosition = 0;
            for (int initpostion = left; initpostion <= right; initpostion++)
            {
                if (HistGram[initpostion] < minHistGram)
                {
                    minHistGram = HistGram[initpostion];
                    minHistPosition = initpostion;
                    //Console.WriteLine("minHistGram=" + minHistGram + " minHistPosition=" + minHistPosition);
                }
            }
            Console.WriteLine("minHistPosition=" + minHistPosition);
            mutlpyY[n_mutlpyY] = minHistPosition;
            n_mutlpyY++;
        }
        return -1;
    }
    ...
    public static int GetMinimumThreshold(int[] HistGram, int[] HistGramS)
    {
        ...
        /*
        for (N = 26; N < 230; N++) //原来是230避免太远的像素点不准确
        {
            accumulatedHand = accumulatedHand + HistGram[N];
            if (accumulatedHand > 500 && isGetPosition == false)
            {
                isGetPosition = true;
                Console.WriteLine("accumulatedHand>500 and N is" + N);
                break;
            }
        }
        */
        //handHistThr = 25450 - 65 * N; //y= -65x+15450 y=y+10000 
    /*
        handHistThr = 25000;
        accumulatedHand = 0;
        for (N = 26; N < 230; N++)  //原来是230避免太远的像素点不准确
        {
            accumulatedHand = accumulatedHand + HistGram[N];
            if (accumulatedHand > handHistThr) break;
        }
        Console.WriteLine("handHistThr=" + handHistThr + "accumulatedHand" + accumulatedHand + "N" + N);
        ...
        while (IsDimodal(HistGramCC, peaks) == false)                                        // 判断是否已经是双峰的图像了      
        {
            ...
            System.Buffer.BlockCopy(HistGramCC, 0, HistGramC, 0, 256 * sizeof(double));
            Iter++;
            if (Iter >= 1000) break;                                                   // 直方图无法平滑为双峰的，返回错误代码
        }
        Console.WriteLine("Iter" + Iter + "peaks" + peaks[0] + " " + peaks[1]);
    ...
    */
    ...
    /// <param name="HistGramS">返回平滑后的直方图</param>
    /// <returns></returns>
    public static int GetMinimumThresholdWithCoord(List<Coord>[] coord_HistGram, List<Coord>[] coord_HistGramSmooth)
    {
        /*
        int Y, Iter = 0;
        int M, N, accumulatedHand = 0;
    ...
    private static bool IsDimodal(double[] HistGram, int[] peaks)       // 检测直方图是否为双峰的
    {
            ...
            if (HistGram[Y - 1] <= HistGram[Y] && HistGram[Y + 1] <= HistGram[Y])
            {
                if ( HistGram[Y]>10 )
                {
                    ...
                    else
                    {
                        peaks[Count] = Y;
                        Count++;
                        //Console.WriteLine("Y" + Y + "HistGram[Y]" + HistGram[Y] + " Count=" + Count);
                    }
                }
            }
        }
        //Console.WriteLine("HistGram[103]" + HistGram[103]+"HistGram[104]" + HistGram[104]+"HistGram[105]" + HistGram[105]);
        if (Count == 2)
            return true;
        else
            return false;
    }
    private static void findPeaks(double[] HistGram, ref int peakCount, int[] peaks)       // 找到直方图中的峰值
    {
        for (int Y = 1; Y < 255; Y++)
        {
            if (HistGram[Y - 1] <= HistGram[Y] && HistGram[Y + 1] <= HistGram[Y])
